...
export class HomePage {
  scans = [];
  scanners = [{ SCANNER_NAME: 'Please Wait...', SCANNER_INDEX: 0, SCANNER_CONNECTION_STATE: true }];
  selectedScanner = 'Please Select...';
  selectedScannerId = -1;
  ean8Decoder = true;   //  Model for decoder
  ean13Decoder = true;  //  Model for decoder
  code39Decoder = true; //  Model for decoder
  code128Decoder = true; //  Model for decoder
  dataWedgeVersion = 'Pre 6.3. Please create & configure profile manually.  See the ReadMe for more details.';
  availableScannersText = 'Requires Datawedge 6.3+';
  activeProfileText = 'Requires Datawedge 6.3+';
  commandResultText = 'Messages from DataWedge will go here';
  uiHideDecoders = true;
  uiDatawedgeVersionAttention = true;
  uiHideSelectScanner = true;
  uiHideShowAvailableScanners = false;
  uiHideCommandMessages = true;
  uiHideFloatingActionButton = true;
  ...
  public constructor(
    ...
    this.platform.ready().then((readySource) => {
      ...
      if (!(this.device.manufacturer.toLowerCase().includes('zebra') ||
        this.device.manufacturer.toLowerCase().includes('motorola solutions'))) {
        this.presentAlert();
      }
      ...
      events.subscribe('data:scan', (data: any) => {
        ...
        const scannedData = data.scanData.extras['com.symbol.datawedge.data_string'];
        const scannedType = data.scanData.extras['com.symbol.datawedge.label_type'];
  ...
  public setDecoders() {
    const paramList = {
      ...
      decoder_ean8: this.ean8Decoder.toString(),
      decoder_ean13: this.ean13Decoder.toString(),
      decoder_code128: this.code128Decoder.toString(),
      decoder_code39: this.code39Decoder.toString(),
    };
